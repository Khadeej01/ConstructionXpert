...
<div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
    <h2 class="text-2xl font-bold text-indigo-700 mb-4">Modifier une Ressource</h2>
    <%
        Resource resource = (Resource) request.getAttribute("resource");
        if (resource == null) {
    %>
    <p class="text-red-500">Ressource non trouvée.</p>
    <% } else { %>
    <form action="ResourceServlet?action=update" method="post" onsubmit="return validateForm()">
        ...
        <div class="mb-4">
            ...
            <select name="taskId" id="taskId" class="w-full p-2 border rounded" required>
                ...
                <option value="<%= tache.getId() %>" <%= resource.getTaskId() == tache.getId() ? "selected" : "" %>>
                    Tâche <%= tache.getId() %> - <%= tache.getDescription() %>
                </option>
                <% } %>
            </select>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="nom" id="nom" class="w-full p-2 border rounded" value="<%= resource.getNom() != null ? resource.getNom() : "" %>" required>
        ...
        <div class="mb-4">
            ...
            <input type="number" name="quantite" id="quantite" class="w-full p-2 border rounded" value="<%= resource.getQuantite() %>" required>
